...
function fetchDataAfterPagination(page, query) {
    // console.log(page);
    fetch(`http://localhost:4500/products/?page=${page}&limit=${ProductLimit}&${query}`).then((res) => res.json()).then((res) => {
...
function getCard(dataId, title, brand, category, rating, price, avatar, quantity) {
    ...
    cartButton.addEventListener("click", (e) => {
        ...
        fetch(`http://localhost:4500/products/delete/${e.target.dataset.id}`, {
            method: "DELETE",
            headers: {
